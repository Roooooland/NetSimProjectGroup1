...
void RFC8569Forwarder::initialize(int stage)
{
    ...
    this->choose_algorithm = par("choose_algorithm");
...
void RFC8569Forwarder::handleMessage(cMessage *msg)
{
    ...
    long oldCacheSize = currentCSSize;
    // self messages
...
void RFC8569Forwarder::processContentObj(ContentObjMsg *contentObjMsg)
{
    ...
    cpsalgorithm->ContentObjhandle(contentObjMsg,csEntry,maximumContentStoreSize,&additionalBytes,&removedBytes);//this->pit.hopsTravelled);
    cs = cpsalgorithm->getCS();

    currentCSSize = cpsalgorithm->getcurrentCSSize();

    pit = cpsalgorithm->getPIT();

    EV_INFO << "\naddbytes: " << additionalBytes
            << "\nremoved: " << removedBytes<<"\n";
    //generate stats
    emit(cacheAdditionsBytesSignal, additionalBytes);
